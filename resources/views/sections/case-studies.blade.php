            ...
            @foreach($section->caseStudies as $case)
                <div class="case-card flex flex-col md:flex-row gap-10 items-end group" data-scroll-speed="0.1">
                    <div class="w-full md:w-3/5 overflow-hidden aspect-video bg-white/5 reveal-card">
                        @if($case->image_path)
                            <img
                                src="{{ \Illuminate\Support\Facades\Storage::disk('uploads')->url($case->image_path) }}"
                                alt="{{ $case->title }}"
                    ...
                    <div class="w-full md:w-2/5 flex flex-col gap-6 pb-10">
                        <div class="flex gap-4">
                            @if($case->tags)
                                @foreach($case->tags as $tag)
                                    <span class="text-[0.6rem] tracking-widest uppercase border border-white/20 px-3 py-1">
                                        {{ $tag }}
                                    </span>
                                @endforeach
                            @endif
                        ...
                        <h3 class="text-4xl md:text-6xl font-black tracking-tighter uppercase group-hover:text-white transition-colors">
                            {{ $case->title }}
                        </h3>
